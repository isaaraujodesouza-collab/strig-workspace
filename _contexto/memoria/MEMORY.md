# Memory Index

- [Tom de copy Kit Lanche Express](feedback_kit-lanche-tom.md) — foco no gestor, não nas credenciais; dados são resposta, não argumento de abertura
- [LPs sem preço — foco em conversão](feedback_landing-page-sem-preco.md) — nunca exibir preço na LP; guiar nível de consciência e confiança até o contato
- [Copy Quatá — correções do cliente](feedback_quata-copy.md) — slides curtos, sem "laudo", sem Advisory em posts de Valuation, encerramento factual sem emoção
- [Regras de escrita da Strig News](feedback_strignews-escrita.md) — sem PL/processo, cargos em português, sem fecho consultivo, título da edição = 3 primeiras notícias combinadas
- [Relatório social com posts turbinados](feedback_relatorio-social-turbinado.md) — sempre avisar no kpi_note quando o período tiver impulsionamento, nunca tentar separar
- [Sem travessão, sem dois-pontos como pausa](feedback_sem-travessao-sem-dois-pontos.md) — nunca usar "—" nem ":" dramático em copy; usar vírgula ou ponto
- [Strig News nunca copia notícia](feedback_strignews-nunca-copiar.md) — headline, linha fina e legenda sempre reescritos no DNA da Strig News, nunca copiados da fonte
- [Copy sem código](feedback_copy-sem-codigo.md) — pedido de reformulação de copy é só texto, nunca HTML/implementação junto
- [Pautas escolhidas no ClickUp](feedback_pautas-escolhidas-clickup.md) — renomear as 7 subtasks pré-existentes (mantendo campo Editoria), notícia sempre em comentário na subtask
- [Legenda: um tópico por linha](feedback_legenda-topico-por-linha.md) — quebrar tópicos/listas em linhas separadas, sem "erro comum:" seco
- [Stories sobe na mão](feedback_stories-upload-manual.md) — nunca incluir stories no agendamento do /publicar-social, mesmo com arte pronta
